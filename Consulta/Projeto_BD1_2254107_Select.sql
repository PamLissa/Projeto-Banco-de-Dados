...
-- nome cirurgiões que não realizaram cirurgias de média complexidade
SELECT C.nome
FROM CIRURGIOES C
WHERE C.crm NOT IN (
    SELECT OC.crm_cirurgiao
    FROM OPERACOES_CIRURGICAS OC
    WHERE OC.complexidade = 'Média Complexidade');

-- nome de cirurgiões que realizaram cirurgia de alta complexidade
SELECT C.nome, C.crm
FROM CIRURGIOES C
WHERE C.crm IN (
    SELECT OC.crm_cirurgiao
...
-- crm e nome de cirurgioes que realizaram operacoes cirurgicas que possuem tempo de duração (horas) maior que o tempo médio (horas)
SELECT C.crm, C.nome
FROM CIRURGIOES C
WHERE C.crm IN (
	SELECT OC.crm_cirurgiao
